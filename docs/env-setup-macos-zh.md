...
```
CC-Launcher-v<版本号>-macOS.dmg    （推荐，Apple Silicon 和 Intel Mac 通用）
CC-Launcher-v<版本号>-macOS.zip    （备用，解压即用）
```

> macOS 构建是 universal app，同一个 `.dmg` 同时覆盖 Apple Silicon（M1/M2/M3/M4）和 Intel Mac。
...
```
~/Library/Application Support/cc-switch/runtime/claude/bin/claude --version
~/Library/Application Support/cc-switch/runtime/codex/bin/codex --version
```
...
2. 删除 `~/Library/Application Support/cc-switch/runtime/claude/` 或 `~/Library/Application Support/cc-switch/runtime/codex/` 里的 npm prefix 内容
3. 删除对应 prefix `bin/` 里的 `claude` / `codex` shim
4. 清空可能残留的空目录
